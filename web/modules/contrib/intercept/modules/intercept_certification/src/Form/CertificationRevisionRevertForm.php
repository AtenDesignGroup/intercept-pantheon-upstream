...
/**
 * Provides a form for reverting a Certification revision.
 *
 * @ingroup intercept_certification
 */
class CertificationRevisionRevertForm extends ConfirmFormBase {
  ...
  public function getFormId() {
    return 'certification_revision_revert_confirm';
  }
  ...
  public function submitForm(array &$form, FormStateInterface $form_state) {
    ...
    $this->logger('content')->notice('Certification: reverted %title revision %revision.', ['%title' => $this->revision->label(), '%revision' => $this->revision->getRevisionId()]);
    $this->messenger()->addMessage(t('Certification %title has been reverted to the revision from %revision-date.', ['%title' => $this->revision->label(), '%revision-date' => $this->dateFormatter->format($original_revision_timestamp)]));
   ...
   *
   * @param \Drupal\intercept_certification\Entity\CertificationInterface $revision
