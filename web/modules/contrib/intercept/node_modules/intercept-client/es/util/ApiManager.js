...
export var apiRegistrar = new Registrar('api');

// Limit file reads to 4 per second.
export var readFileLimiter = new Bottleneck({
  maxConcurrent: 4,
  minTime: 1000 / 4
});
// Limit reads to 6 per second.
export var readLimiter = new Bottleneck({
  maxConcurrent: 6,
  minTime: 1000 / 6
});
// Limit writes to 2 at a time and 3 per second.
export var writeLimiter = new Bottleneck({
  maxConcurrent: 2,
  minTime: 1000 / 3
});
// Limit validates to 1 at a time and 3 per second.
export var validateLimiter = new Bottleneck({
  maxConcurrent: 1,
  minTime: 1000 / 3
});
...
export function handleValidateSuccessResponse(resp, items, dispatch, resource) {
  return resp.json().then(function (json) {
    ...
    var manifest = reduce(json.states, function (accumulator, remoteItem, key) {
      // Locally stored item;
...
export function processIncludes(dispatch) {
  return function (includes) {
    var resources = groupBy(includes, function (record) {
      return record.type;
    });
    forEach(resources, function (records, resource) {
...
export var ApiManager = function () {
  ...
  _createClass(ApiManager, [{
    ...
    value: function fetchAll() {
      ...
      var currentFetch = setLatestFetch(uuidv4());
    ...
    value: function updateRelationshipsIfNeeded(dispatch, localData, remoteData) {
      ...
      var dirtyRelationships = filter(relationships, function (r) {
        return (
    ...
    value: function getEndpointFilters(filters) {
      return reduce(filters, function (query, value, key) {
        var output = assign({}, query);
    ...
    value: function getEndpointSort(sort) {
      var output = reduce(sort, function (query, value) {
    ...
    value: function getEndpointFields(fields) {
      ...
      return reduce(fields, function (query, value, key) {
        return assign(query, _defineProperty({}, 'fields[' + key + ']', value.join(',')));
      }, {});
    }
...
function clearItemsErrors(items) {
  return mapValues(items, function (item) {
...
function mergeTranslation(items, data, mergeStrategy, langcode) {
  var output = assign({}, items[data.uuid]);
  var translations = 'translations' in output.data ? output.data.translations : {};
  output.data.translations = assign(translations, _defineProperty({}, langcode, data));
...
export function dataReducer() {
  ...
  return assign({}, state, _defineProperty({}, id, item));
}
